...
# def book_appointment(request, extra=None):
#     if extra:
#         # Optional handling of the extra path parameter if needed
#         print(f"Extra path parameter received: {extra}")
#     return render(request, 'chatbot/index.html')
...
#             receiver_email = "shwed1112@gmail.com"  # Replace with actual doctor's email
...
CALENDLY_USER = "your-calendly-user"
CALENDLY_EVENT_TYPE = "doctor-s-consultation"
...
def translate_view(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            text = data.get('text', '')
            from_lang = data.get('from_lang', 'hi')
...
def upload_prescription(request):
    if request.method == 'POST':
        if 'file' not in request.FILES:
            return JsonResponse({'success': False, 'message': 'No file uploaded. Please select a file.'})
        ...
        if form.is_valid():
            ...
            # Process file (e.g., extract text and generate recommendations)
            extracted_text = extract_text_from_file(file_path)
            recommendations = recommend_medicine(extracted_text)
            ...
            return JsonResponse({'success': False, 'message': 'Invalid file.', 'errors': form.errors})
    return JsonResponse({'success': False, 'message': 'Invalid request method.'})
...
def send_email_view(request):
    if request.method == 'POST':
        ...
        translated_text = translate_text(original_text, from_lang='hi', to_lang='en')

        if translated_text:
            sender_email = settings.EMAIL_HOST_USER
            sender_password = settings.EMAIL_HOST_PASSWORD
            receiver_email = "shwed1112@gmail.com"
...
def extract_text_from_image(image_path):
    try:
        ...
        logger.error("Tesseract OCR not found. Ensure it's installed and the path is set correctly.")
        raise
    except Exception as e:
